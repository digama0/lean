...
static expr parse_Type(parser & p, unsigned, expr const *, pos_info const & pos) {
    ...
    p.set_ast_pexpr(p.new_ast("Type", pos).push(id).m_id, r);
    return r;
}
...
static expr parse_Sort(parser & p, unsigned, expr const *, pos_info const & pos) {
    ...
    p.set_ast_pexpr(p.new_ast("Sort", pos).push(id).m_id, r);
    return r;
}
...
static expr parse_let(parser & p, pos_info const & pos, ast_data & group, bool in_do_block);
static expr parse_do(parser & p, ast_data & group, bool has_braces);
static expr parse_let_body(parser & p, pos_info const & pos, ast_id last, ast_data & group, bool in_do_block) {
    if (in_do_block) {
        if (p.curr_is_token(get_in_tk())) {
            p.next();
            ast_id args = p.new_ast("args", pos).push(last).m_id;
            expr e = p.parse_expr();
            p.set_ast_pexpr(p.new_ast(get_let_tk(), pos).push(args).push(p.get_id(e)).m_id, e);
            return e;
        } else {
        ...
        expr e = p.parse_expr();
        p.set_ast_pexpr(p.new_ast(get_let_tk(), pos).push(group.m_id).push(p.get_id(e)).m_id, e);
        return e;
    }
}
...
static expr parse_let(parser & p, pos_info const & pos, ast_data & group, bool in_do_block) {
    ...
    if (!in_do_block && (nota = p.parse_local_notation_decl())) {
        ...
        expr b = parse_let_body(p, pos, var.m_id, group, in_do_block);
        ...
        var.push(p.get_id(lhs)).push(0).push(0).push(p.get_id(value));
        for (expr const & l : new_locals)
            p.add_local(l);
        expr body  = parse_let_body(p, pos, var.m_id, group, in_do_block);
...
static expr parse_do(parser & p, ast_data & group, bool has_braces) {
    ...
    while (true) {
        if (p.curr_is_token(get_let_tk())) {
            ...
            es.push_back(parse_let(p, pos, group, in_do_block));
            break;
        } else {
            auto lhs_pos = p.pos();
...
static expr parse_do_expr(parser & p, unsigned, expr const *, pos_info const & pos) {
    ...
    return parse_do(p, data, has_braces);
}
...
static expr parse_have(parser & p, unsigned, expr const *, pos_info const & pos) {
    ...
    if (p.curr_is_identifier()) {
        id = p.get_name_val();
        auto& id_ast = p.new_ast("ident", p.pos());
        p.next();
        if (p.curr_is_token(get_colon_tk())) {
            p.next();
            prop      = p.parse_expr();
...
static expr parse_suffices(parser & p, unsigned, expr const *, pos_info const & pos) {
    ...
    if (p.curr_is_identifier()) {
        id = p.get_name_val();
        auto& id_ast = p.new_ast("ident", prop_pos);
        p.next();
        if (p.curr_is_token(get_colon_tk())) {
            p.next();
            from = p.parse_expr();
...
static expr parse_if_then_else(parser & p, unsigned, expr const *, pos_info const & pos) {
    ...
    auto& data = p.new_ast("if", pos).push(id);
...
static expr parse_quoted_name(parser & p, unsigned, expr const *, pos_info const & pos) {
    ...
    if (resolve) {
        ...
        expr e = p.id_to_expr(id, p.new_ast("ident", pos), resolve_only);
        if (is_constant(e)) {
            id = const_name(e);
        } else if (is_local(e)) {
            id = mlocal_pp_name(e);
...
static expr parse_lambda_core(parser & p, ast_data & fun, ast_data & group, pos_info const & pos);

static expr parse_lambda_binder(parser & p, ast_data & fun, ast_data & group, pos_info const & pos) {
    parser::local_scope scope1(p);
    buffer<expr> locals;
    auto new_env = p.parse_binders(&group, locals, 0);
    ...
    if (p.curr_is_token(get_comma_tk())) {
        ...
        body = parse_lambda_core(p, fun, group, pos);
    } else {
...
static expr parse_lambda_constructor(parser & p, ast_data & fun, ast_data & group, pos_info const & ini_pos) {
    ...
    if (p.curr_is_token(get_comma_tk())) {
        ...
        body = parse_lambda_core(p, fun, group, ini_pos);
    }
    match_definition_scope match_scope(p.env());
...
static expr parse_lambda_core(parser & p, ast_data & fun, ast_data & group, pos_info const & pos) {
    if (p.curr_is_token(get_langle_tk())) {
        return parse_lambda_constructor(p, fun, group, pos);
    } else {
        return parse_lambda_binder(p, fun, group, pos);
    }
}

static expr parse_lambda(parser & p, unsigned, expr const *, pos_info const & pos) {
    auto& group = p.new_ast("args", p.pos());
    auto& fun = p.new_ast("fun", pos).push(group.m_id);
    expr r = parse_lambda_core(p, fun, group, pos);
...
static expr parse_assume(parser & p, unsigned, expr const *, pos_info const & pos) {
    ...
    if (p.curr_is_token(get_colon_tk())) {
        ...
        p.next();
        expr prop = p.parse_expr();
        auto& vars = p.new_ast("vars", p.pos()).push(0);
        auto& binder = p.new_ast(name("binder").append_after(0U), p.pos())
            .push(vars.m_id).push(0).push(p.get_id(prop));
        auto& group = p.new_ast("args", p.pos()).push(binder.m_id);
        p.check_token_next(get_comma_tk(), "invalid 'assume', ',' expected");
        parser::local_scope scope(p);
        expr l = p.save_pos(mk_local(get_this_tk(), prop), pos);
        p.add_local(l);
        expr body = p.parse_expr();
        id = p.new_ast("assume", pos).push(group.m_id).push(p.get_id(body)).m_id;
        r = p.save_pos(Fun(l, body, p), pos);
    } else {
        auto& group = p.new_ast("args", p.pos());
        auto& fun = p.new_ast("assume", pos).push(group.m_id);
        id = fun.m_id;
        r = parse_lambda_core(p, fun, group, pos);
    }
...
static expr parse_infix_paren(parser & p, list<notation::accepting> const & accs, pos_info const & pos) {
    ...
    p.next();
    auto& data = p.new_ast("infix_paren", pos).push(0);
    ...
    if (length(accs) > 1) {
        auto& choices = p.new_ast("choices", pos);
        data.m_children[0] = p.new_ast("choice", pos).push(choices.m_id).m_id;
        for (auto& a : accs)
            choices.push(p.new_ast("notation", pos, a.get_name()).m_id);
    } else {
        data.m_children[0] = p.new_ast("notation", pos, head(accs).get_name()).m_id;
    }
    p.set_ast_pexpr(data.m_id, r);
    return r;
}
...
static expr parse_lambda_cons(parser & p, unsigned, expr const *, pos_info const & pos) {
    ...
    expr r = p.save_pos(mk_choice(cs.size(), cs.data()), pos);
    auto& data = p.new_ast("(infix", pos);
    if (length(accs) > 1) {
        auto& choices = p.new_ast("choices", pos);
        data.push(p.new_ast("choice", pos).push(choices.m_id).m_id);
        for (auto& a : accs)
            choices.push(p.new_ast("notation", pos, a.get_name()).m_id);
    } else {
        data.push(p.new_ast("notation", pos, head(accs).get_name()).m_id);
    }
    data.push(0);
    p.set_ast_pexpr(data.m_id, r);
    return r;
