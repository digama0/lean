...
static expr mk_choice(parser & p, const buffer<expr> & es) {
    ...
    if (es.size() == 1)
        return es[0];
    expr r = mk_choice(es.size(), es.data());
...
expr parser::parse_numeral_expr(bool user_notation) {
    ...
    set_ast_pexpr(new_ast("num", p, n.to_string()).m_id, r);
    return r;
}
...
void parser::parse_command(cmd_meta const & meta) {
    ...
    if (auto it = cmds().find(cmd_name)) {
        push_command(new_ast(cmd_name, pos()).m_id);
...
void parser::parse_mod_doc_block() {
    ...
    push_command(new_ast("mdoc", pos, doc).m_id);
...
bool parser::parse_imports(unsigned & fingerprint, std::vector<module_name> & imports) {
    ...
    if (curr_is_token(get_prelude_tk())) {
        push_command(new_ast("prelude", pos()).m_id);
        next();
        prelude = true;
    }
    if (!prelude) {
