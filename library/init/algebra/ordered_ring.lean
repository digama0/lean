...
(mul_le_mul_of_le_left :  ∀ a b c : α, a ≤ b → 0 ≤ c → c * a ≤ c * b)
(mul_le_mul_of_le_right : ∀ a b c : α, a ≤ b → 0 ≤ c → a * c ≤ b * c)
(mul_lt_mul_of_lt_left :  ∀ a b c : α, a < b → 0 < c → c * a < c * b)
(mul_lt_mul_of_lt_right : ∀ a b c : α, a < b → 0 < c → a * c < b * c)
...
lemma mul_le_mul_of_le_left {a b c : α} (h₁ : a ≤ b) (h₂ : 0 ≤ c) : c * a ≤ c * b :=
ordered_semiring.mul_le_mul_of_le_left a b c h₁ h₂

lemma mul_le_mul_of_le_right {a b c : α} (h₁ : a ≤ b) (h₂ : 0 ≤ c) : a * c ≤ b * c :=
ordered_semiring.mul_le_mul_of_le_right a b c h₁ h₂

lemma mul_lt_mul_of_lt_left {a b c : α} (h₁ : a < b) (h₂ : 0 < c) : c * a < c * b :=
ordered_semiring.mul_lt_mul_of_lt_left a b c h₁ h₂

lemma mul_lt_mul_of_lt_right {a b c : α} (h₁ : a < b) (h₂ : 0 < c) : a * c < b * c :=
ordered_semiring.mul_lt_mul_of_lt_right a b c h₁ h₂

-- TODO: there are four variations, depending on which variables we assume to be nonneg
lemma mul_le_mul {a b c d : α} (hac : a ≤ c) (hbd : b ≤ d) (nn_b : 0 ≤ b) (nn_c : 0 ≤ c) : a * b ≤ c * d :=
calc
  a * b ≤ c * b : mul_le_mul_of_le_right hac nn_b
    ... ≤ c * d : mul_le_mul_of_le_left hbd nn_c

lemma mul_nonneg {a b : α} (ha : a ≥ 0) (hb : b ≥ 0) : a * b ≥ 0 :=
have h : 0 * b ≤ a * b, from mul_le_mul_of_le_right ha hb,
by rwa [zero_mul] at h

lemma mul_nonpos_of_nonneg_of_nonpos {a b : α} (ha : a ≥ 0) (hb : b ≤ 0) : a * b ≤ 0 :=
have h : a * b ≤ a * 0, from mul_le_mul_of_le_left hb ha,
by rwa mul_zero at h

lemma mul_nonpos_of_nonpos_of_nonneg {a b : α} (ha : a ≤ 0) (hb : b ≥ 0) : a * b ≤ 0 :=
have h : a * b ≤ 0 * b, from mul_le_mul_of_le_right ha hb,
by rwa zero_mul at h

lemma mul_lt_mul {a b c d : α} (hac : a < c) (hbd : b ≤ d) (pos_b : 0 < b) (nn_c : 0 ≤ c) :  a * b < c * d :=
calc
  a * b < c * b : mul_lt_mul_of_lt_right hac pos_b
    ... ≤ c * d : mul_le_mul_of_le_left hbd nn_c

lemma mul_lt_mul' {a b c d : α} (h1 : a ≤ c) (h2 : b < d) (h3 : b ≥ 0) (h4 : c > 0) :
       a * b < c * d :=
calc
   a * b ≤ c * b : mul_le_mul_of_le_right h1 h3
     ... < c * d : mul_lt_mul_of_lt_left h2 h4

lemma mul_pos {a b : α} (ha : a > 0) (hb : b > 0) : a * b > 0 :=
have h : 0 * b < a * b, from mul_lt_mul_of_lt_right ha hb,
by rwa zero_mul at h

lemma mul_neg_of_pos_of_neg {a b : α} (ha : a > 0) (hb : b < 0) : a * b < 0 :=
have h : a * b < a * 0, from mul_lt_mul_of_lt_left hb ha,
by rwa mul_zero at h

lemma mul_neg_of_neg_of_pos {a b : α} (ha : a < 0) (hb : b > 0) : a * b < 0 :=
have h : a * b < 0 * b, from mul_lt_mul_of_lt_right ha hb,
by rwa zero_mul at  h

lemma mul_self_le_mul_self_of_le {a b : α} (h1 : 0 ≤ a) (h2 : a ≤ b) : a * a ≤ b * b :=
mul_le_mul h2 h2 h1 (le_trans h1 h2)

lemma mul_self_lt_mul_self_of_lt {a b : α} (h1 : 0 ≤ a) (h2 : a < b) : a * a < b * b :=
...
lt_of_not_ge
  (assume h1 : b ≤ a,
   have h2 : c * b ≤ c * a, from mul_le_mul_of_le_left h1 hc,
   not_lt_of_ge h2 h)

lemma lt_of_mul_lt_mul_right {a b c : α} (h : a * c < b * c) (hc : c ≥ 0) : a < b :=
lt_of_not_ge
  (assume h1 : b ≤ a,
   have h2 : b * c ≤ a * c, from mul_le_mul_of_le_right h1 hc,
   not_lt_of_ge h2 h)

lemma le_of_mul_le_mul_left {a b c : α} (h : c * a ≤ c * b) (hc : c > 0) : a ≤ b :=
le_of_not_gt
  (assume h1 : b < a,
   have h2 : c * b < c * a, from mul_lt_mul_of_lt_left h1 hc,
   not_le_of_gt h2 h)

lemma le_of_mul_le_mul_right {a b c : α} (h : a * c ≤ b * c) (hc : c > 0) : a ≤ b :=
le_of_not_gt
  (assume h1 : b < a,
   have h2 : b * c < a * c, from mul_lt_mul_of_lt_right h1 hc,
   not_le_of_gt h2 h)

@[simp] lemma mul_le_mul_left {a b c : α} (h : 0 < c) : c * a ≤ c * b ↔ a ≤ b :=
⟨λ h', le_of_mul_le_mul_left h' h, λ h', mul_le_mul_of_le_left h' (le_of_lt h)⟩

@[simp] lemma mul_le_mul_right {a b c : α} (h : 0 < c) : a * c ≤ b * c ↔ a ≤ b :=
⟨λ h', le_of_mul_le_mul_right h' h, λ h', mul_le_mul_of_le_right h' (le_of_lt h)⟩

@[simp] lemma mul_lt_mul_left {a b c : α} (h : 0 < c) : c * a < c * b ↔ a < b :=
⟨λ h', lt_of_mul_lt_mul_left h' (le_of_lt h), λ h', mul_lt_mul_of_lt_left h' h⟩

@[simp] lemma mul_lt_mul_right {a b c : α} (h : 0 < c) : a * c < b * c ↔ a < b :=
⟨λ h', lt_of_mul_lt_mul_right h' (le_of_lt h), λ h', mul_lt_mul_of_lt_right h' h⟩
...
{ s with
  ...
  add_le_add_left        := @add_le_add_left α _,
  mul_le_mul_of_le_left  := λ a b c ab c0, sub_nonneg.1 $
    by rw ← mul_sub_left_distrib; exact ordered_ring.mul_nonneg _ _ c0 (sub_nonneg.2 ab),
  mul_le_mul_of_le_right := λ a b c ab c0, sub_nonneg.1 $
    by rw ← mul_sub_right_distrib; exact ordered_ring.mul_nonneg _ _ (sub_nonneg.2 ab) c0,
  mul_lt_mul_of_lt_left  := λ a b c ab c0, sub_pos.1 $
    by rw ← mul_sub_left_distrib; exact ordered_ring.mul_pos _ _ c0 (sub_pos.2 ab),
  mul_lt_mul_of_lt_right := λ a b c ab c0, sub_pos.1 $
    by rw ← mul_sub_right_distrib; exact ordered_ring.mul_pos _ _ (sub_pos.2 ab) c0,
  add_lt_add_left        := @add_lt_add_left α _}
...
have -c * b ≤ -c * a,     from mul_le_mul_of_le_left h this,
...
have -c ≥ 0,              from neg_nonneg_of_nonpos hc,
have b * -c ≤ a * -c,     from mul_le_mul_of_le_right h this,
...
have -c * b < -c * a,     from mul_lt_mul_of_lt_left h this,
have -(c * b) < -(c * a), by rwa [← neg_mul_eq_neg_mul, ← neg_mul_eq_neg_mul] at this,
lt_of_neg_lt_neg this

lemma mul_lt_mul_of_neg_right {a b c : α} (h : b < a) (hc : c < 0) : a * c < b * c :=
have -c > 0,              from neg_pos_of_neg hc,
have b * -c < a * -c,     from mul_lt_mul_of_lt_right h this,
have -(b * c) < -(a * c), by rwa [← neg_mul_eq_mul_neg, ← neg_mul_eq_mul_neg] at this,
lt_of_neg_lt_neg this
...
{ s with
  ...
  add_right_cancel       := @add_right_cancel α _,
  add_le_add_left        := @add_le_add_left α _,
  mul_le_mul_of_le_left  := @mul_le_mul_of_le_left α _,
  mul_le_mul_of_le_right := @mul_le_mul_of_le_right α _,
  mul_lt_mul_of_lt_left  := @mul_lt_mul_of_lt_left α _,
  mul_lt_mul_of_lt_right := @mul_lt_mul_of_lt_right α _,
...
have h2 : -(c * b) < -(c * a), from neg_lt_neg_of_lt h,
...
begin
  have this := neg_lt_neg_of_lt (@zero_lt_one α _),
...
have h' : a * c ≤ b * c, from calc
     a * c ≤ b : h
       ...
       ... ≤ b * c : mul_le_mul_of_le_left hc hb,
le_of_mul_le_mul_right h' (lt_of_lt_of_le zero_lt_one hc)

lemma nonneg_le_nonneg_of_squares_le {a b : α} (hb : b ≥ 0) (h : a * a ≤ b * b) : a ≤ b :=
le_of_not_gt (λhab, not_le_of_gt (mul_self_lt_mul_self_of_lt hb hab) h)

lemma mul_self_le_mul_self {a b : α} (h1 : 0 ≤ a) (h2 : 0 ≤ b) : a ≤ b ↔ a * a ≤ b * b :=
⟨mul_self_le_mul_self_of_le h1, nonneg_le_nonneg_of_squares_le h2⟩

lemma mul_self_lt_mul_self {a b : α} (h1 : 0 ≤ a) (h2 : 0 ≤ b) : a < b ↔ a * a < b * b :=
le_iff_le_iff_lt_iff_lt.1 (mul_self_le_mul_self h2 h1)

end linear_ordered_ring

class linear_ordered_comm_ring (α : Type u) extends linear_ordered_ring α, comm_monoid α

lemma linear_ordered_comm_ring.eq_zero_or_eq_zero_of_mul_eq_zero [linear_ordered_comm_ring α]
...
let s : linear_ordered_semiring α := linear_ordered_ring.to_linear_ordered_semiring in
{ d with
  ...
  add_le_add_left        := @linear_ordered_semiring.add_le_add_left α s,
  add_lt_add_left        := @linear_ordered_semiring.add_lt_add_left α s,
  mul_le_mul_of_le_left  := @linear_ordered_semiring.mul_le_mul_of_le_left α s,
  mul_le_mul_of_le_right := @linear_ordered_semiring.mul_le_mul_of_le_right α s,
  mul_lt_mul_of_lt_left  := @linear_ordered_semiring.mul_lt_mul_of_lt_left α s,
  mul_lt_mul_of_lt_right := @linear_ordered_semiring.mul_lt_mul_of_lt_right α s }

instance decidable_linear_ordered_comm_ring.to_decidable_linear_ordered_comm_group [d : decidable_linear_ordered_comm_ring α] :
