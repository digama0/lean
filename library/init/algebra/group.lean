...
@[simp] lemma mul_left_inj [left_cancel_semigroup α] {a b c : α} : a * b = a * c ↔ b = c :=
⟨mul_left_cancel, congr_arg _⟩

@[simp] lemma mul_right_inj [right_cancel_semigroup α] {a b c : α} : b * a = c * a ↔ b = c :=
...
@[simp] lemma inv_inj [group α] {a b : α} : a⁻¹ = b⁻¹ ↔ a = b :=
⟨λ h, by rw ← inv_inv a; simp [h], congr_arg _⟩

lemma eq_of_inv_eq_inv [group α] {a b : α} : a⁻¹ = b⁻¹ → a = b :=
inv_inj.1

@[simp] lemma inv_eq_one [group α] {a : α} : a⁻¹ = 1 ↔ a = 1 :=
by rw [← @inv_inj _ _ a 1, one_inv]

@[simp] lemma inv_ne_one [group α] {a : α} : a⁻¹ ≠ 1 ↔ a ≠ 1 :=
...
@[simp] lemma mul_inv_cancel_left [group α] (a b : α) : a * (a⁻¹ * b) = b :=
by rw [← mul_assoc, mul_right_inv, one_mul]

@[simp] lemma mul_inv_cancel_right [group α] (a b : α) : a * b * b⁻¹ = a :=
...
lemma eq_inv_iff_eq_inv [group α] {a b : α} : a = b⁻¹ ↔ b = a⁻¹ :=
⟨eq_inv_of_eq_inv, eq_inv_of_eq_inv⟩

lemma inv_eq_iff_inv_eq [group α] {a b : α} : a⁻¹ = b ↔ b⁻¹ = a :=
by rw [eq_comm, @eq_comm _ _ a, eq_inv_iff_eq_inv]

lemma mul_eq_one_iff_eq_inv [group α] {a b : α} : a * b = 1 ↔ a = b⁻¹ :=
have a * b = b⁻¹ * b ↔ a = b⁻¹, from mul_right_inj,
by rwa mul_left_inv at this

lemma mul_eq_one_iff_inv_eq [group α] {a b : α} : a * b = 1 ↔ a⁻¹ = b :=
by rw [mul_eq_one_iff_eq_inv, eq_inv_iff_eq_inv, eq_comm]

@[simp] lemma eq_mul_inv_iff_mul_eq [group α] {a b c : α} : a = b * c⁻¹ ↔ a * c = b :=
⟨λ h, by simp [h], λ h, by simp [h.symm]⟩

@[simp] lemma eq_inv_mul_iff_mul_eq [group α] {a b c : α} : a = b⁻¹ * c ↔ b * a = c :=
⟨λ h, by simp [h], λ h, by simp [h.symm]⟩

@[simp] lemma inv_mul_eq_iff_eq_mul [group α] {a b c : α} : a⁻¹ * b = c ↔ b = a * c :=
⟨λ h, by simp [h.symm], λ h, by simp [h]⟩

@[simp] lemma mul_inv_eq_iff_eq_mul [group α] {a b c : α} : a * b⁻¹ = c ↔ a = c * b :=
⟨λ h, by simp [h.symm], λ h, by simp [h]⟩
...
lemma eq_mul_inv_of_mul_eq [group α] {a b c : α} : a * c = b → a = b * c⁻¹ :=
eq_mul_inv_iff_mul_eq.2

lemma eq_inv_mul_of_mul_eq [group α] {a b c : α} : b * a = c → a = b⁻¹ * c :=
eq_inv_mul_iff_mul_eq.2

lemma inv_mul_eq_of_eq_mul [group α] {a b c : α} : b = a * c → a⁻¹ * b = c :=
inv_mul_eq_iff_eq_mul.2

lemma mul_inv_eq_of_eq_mul [group α] {a b c : α} : a = c * b → a * b⁻¹ = c :=
mul_inv_eq_iff_eq_mul.2

lemma eq_mul_of_mul_inv_eq [group α] {a b c : α} : a * c⁻¹ = b → a = b * c :=
mul_inv_eq_iff_eq_mul.1

lemma eq_mul_of_inv_mul_eq [group α] {a b c : α} : b⁻¹ * a = c → a = b * c :=
inv_mul_eq_iff_eq_mul.1

lemma mul_eq_of_eq_inv_mul [group α] {a b c : α} : b = a⁻¹ * c → a * b = c :=
eq_inv_mul_iff_mul_eq.1

lemma mul_eq_of_eq_mul_inv [group α] {a b c : α} : a = c * b⁻¹ → a * b = c :=
eq_mul_inv_iff_mul_eq.1
...
run_cmd transport_multiplicative_to_additive
  [/- map operations -/
   ...
   (`mul_inv, `neg_add),
   (`inv_inj, `neg_inj),
   (`eq_of_inv_eq_inv, `eq_of_neg_eq_neg),
...
@[simp] lemma sub_left_inj [add_group α] {a b c : α} : a - b = a - c ↔ b = c :=
add_left_inj.trans neg_inj

@[simp] lemma sub_right_inj [add_group α] {a b c : α} : b - a = c - a ↔ b = c :=
add_right_inj

lemma add_sub_assoc [add_group α] (a b c : α) : a + b - c = a + (b - c) :=
by rw [sub_eq_add_neg, add_assoc, ←sub_eq_add_neg]

lemma sub_eq_zero [add_group α] {a b : α} : a - b = 0 ↔ a = b :=
have a - b + b = 0 + b ↔ a - b = 0, from add_right_inj,
by rw [sub_add_cancel, zero_add] at this; exact this.symm

lemma eq_of_sub_eq_zero [add_group α] {a b : α} : a - b = 0 → a = b :=
sub_eq_zero.1

lemma sub_eq_zero_of_eq [add_group α] {a b : α} : a = b → a - b = 0 :=
sub_eq_zero.2
...
lemma sub_ne_zero [add_group α] {a b : α} : a - b ≠ 0 ↔ a ≠ b :=
not_congr sub_eq_zero

lemma sub_ne_zero_of_ne [add_group α] {a b : α} : a ≠ b → a - b ≠ 0 :=
sub_ne_zero.2
...
@[simp] lemma eq_sub_iff_add_eq [add_group α] {a b c : α} : a = b - c ↔ a + c = b :=
by split; intro h; simp [h]

lemma sub_eq_iff_eq_add [add_group α] {a b c : α} : a - b = c ↔ a = c + b :=
by split; intro h; simp * at *

lemma eq_sub_of_add_eq [add_group α] {a b c : α} : a + c = b → a = b - c :=
eq_sub_iff_add_eq.2

lemma sub_eq_of_eq_add [add_group α] {a b c : α} : a = c + b → a - b = c :=
sub_eq_iff_eq_add.2

lemma eq_add_of_sub_eq [add_group α] {a b c : α} : a - c = b → a = b + c :=
sub_eq_iff_eq_add.1

lemma add_eq_of_eq_sub [add_group α] {a b c : α} : a = c - b → a + b = c :=
eq_sub_iff_add_eq.1
...
lemma sub_eq_neg_add [add_comm_group α] (a b : α) : a - b = -b + a :=
by simp
...
lemma eq_sub_iff_add_eq' [add_comm_group α] {a b c : α} : a = b - c ↔ c + a = b :=
by rw [eq_sub_iff_add_eq, add_comm]

lemma sub_eq_iff_eq_add' [add_comm_group α] {a b c : α} : a - b = c ↔ a = b + c :=
by rw [sub_eq_iff_eq_add, add_comm]

lemma eq_sub_of_add_eq' [add_comm_group α] {a b c : α} : c + a = b → a = b - c :=
eq_sub_iff_add_eq'.2

lemma sub_eq_of_eq_add' [add_comm_group α] {a b c : α} : a = b + c → a - b = c :=
sub_eq_iff_eq_add'.2

lemma eq_add_of_sub_eq' [add_comm_group α] {a b c : α} : a - b = c → a = b + c :=
sub_eq_iff_eq_add'.1

lemma add_eq_of_eq_sub' [add_comm_group α] {a b c : α} : b = c - a → a + b = c :=
eq_sub_iff_add_eq'.1
